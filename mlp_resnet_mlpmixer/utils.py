...
def cross_entropy_loss(logits: torch.Tensor, labels: torch.Tensor):
    ...
    :param logits: [batch_size, num_class]  [30, 10]
    :param labels: [batch_size]             [30]
    :return loss 
    """
    # Took softmax class-wise
    logits_probs = torch.nn.functional.log_softmax(logits, dim=1)
    ce = -torch.mean(logits_probs[range(labels.shape[0]), labels])
    return ce
